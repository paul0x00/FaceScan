...
后端按职责拆分为 `FaceScanCamera`、`FaceScanConfig`、`FaceScanReconstruction`、`FaceScanService` 和 `FaceScanDatabase` 动态库；`facescan_common` 是供这些模块内部复用的静态基础库。`FaceScanBackend` 负责加载配置、解析启动参数并调用 `FaceScanService`，测试按需链接各业务模块。Windows 产物为 DLL，macOS 产物为 dylib。
...
- `backend/src/FaceScanBackend/main.cpp` 仅是进程入口。`backend/src/FaceScanService/http_server.*` 负责 Boost.Beast/Asio 的 HTTP 监听，采用每连接一线程的 accept 循环。`backend/src/FaceScanService/app.*`（`App::handle`）负责全部路由与响应生成。
